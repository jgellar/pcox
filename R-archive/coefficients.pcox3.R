...
coefficients.pcox <- function(object, select=NULL, se=TRUE, use.var2=FALSE,
  ...
  if (se) {
    Vp <- if (use.var2) object$var2 else object$var
    fakegam$Vp <- Vp
  }
  ...
  coef <- lapply(select, function(i) {
    ...
    if (length(smooth.i$term) < 3) {
      ...
      n.is <- c(n.i, n2.i, rep(n3.i, length(smooth.i$term)-2))
      coef.i <- do.call(expand.grid, mapply(function(x,y) {
        seq(min(x), max(x), length=y)
      }, sdat.i[smooth.i$term], n.is))
      if (smooth.i$by!="NA")
      ...
      coef.i$value <- pmat %*% fakegam$coefficients[first:last]
      ...
      if (se) { ## get standard errors for fit
        ...
        seWithMean <- list(...)$seWithMean
        if (all(!is.null(seWithMean), seWithMean, attr(smooth.i, "nCons")>0)) {
          ...
          coef.i$se  <-
            sqrt(pmax(0, rowSums((pmat %*% object$var[ first:last, first:last, drop=FALSE]) * pmat)))
          if (!is.null(object$var2))
            coef.i$se2 <-
            sqrt(pmax(0, rowSums((pmat %*% object$var2[first:last, first:last, drop=FALSE]) * pmat)))
        }
