#' Paramteric interaction basis constructor
#' 
#' The \code{pi} basis is appropriate for smooths of multiple variables. Its
#' purpose is to
#' parameterize the way in which the basis changes with one of those variables.
#' For example, suppose the smooth is over three variables, \eqn{x}, \eqn{y},
#' and \eqn{t}, and we want to parameterize the effect of \eqn{t}. Then the
#' \code{pi} basis will assume \eqn{f(x,y,t) = \sum_k g_k(t)*f_k(x,y)}, where
#' the \eqn{g_k(t)} functions are pre-specified and the \eqn{f_k(x,y)} functions
#' are estimated using a biariate basis. An example of a parametric interaction
#' is a linear interaction, which would take the form
...
# \code{s(x, t, bs="pb", xt=list("linear", bs="ps", k=15))}.
# 
...
# type, which can only be set within \code{xt} because \code{bs="pb"} within
# \code{s()}. If the basis option is defined in both places, the setting
# within \code{xt} will be used. For example, either
# \code{s(x, t, bs="pb", k=15, xt=list("linear", bs="ps"))} or
# \code{s(x, t, bs="pb", k=20, xt=list("linear", bs="ps", k=15))}
...
# argument will already be specified as \code{bs="pb"}.
# 
# For example, to define a linear interaction basis using p-splines
# of dimension 15, the code could either be
# \code{s(x, t, bs="pb", xt=list("linear", bs="ps", k=15))} or
# \code{s(x, t, bs="pb", k=15, xt=list("linear", bs="ps"))}. If the default
# thin-plate regression splines are desired, the specification could be
# \code{s(x, t, bs="pb", k=15, xt="linear")}.
# 
...
smooth.construct.pi.smooth.spec <- function(object, data, knots) {
  ...
  t_X <-  vapply(tf, function(f)
    do.call(f, list(t=tvar)), numeric(n))
