#' Compute a Survival Curve from a \code{pcox} model
#' 
#' Computes the predicted survivor function for a \code{pcox} model. This
#' function is a modified version of \code{\link[survival]{survfit.coxph}}.
#' 
#' @param formula a \code{pcox} object
#' 
#' 
#' 
...
#' @seealso \code{\link[survival]{survfit}}, \code{\link[survival]{survfit.coxph}},
#'   \code{\link{pcox}}

survfit.pcox <- function(formula, newdata, se.fit = TRUE, conf.int = 0.95,
                         individual = FALSE,  type, vartype,
                         conf.type = c("log", "log-log", "plain", "none"),
                         censor = TRUE, id, na.action = na.pass, ...) {
  Call <- match.call()
  Call[[1]] <- as.name("survfit")
  object <- formula
  
  # survtype - method for survival estimation (baseline hazard)
  if (missing(type)) {
  ...
  # vartype - method for estimating SE
  if (missing(vartype)) {
    vartype <- survtype
  }
  else {
  ...
  # conf.type - scale for confidence intervals?
  if (!se.fit) 
    conf.type <- "none"
  ...
  # Extract survival object (y) and design matrix (x)
  ...
  # Set up id/individual variables
  missid <- missing(id)
  if (!missid) 
    individual <- TRUE
