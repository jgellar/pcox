permalgorithm2 <- function (numSubjects, maxTime, Xmat, XmatNames = NULL, eventRandom = NULL, 
          censorRandom = NULL, betas, groupByD = FALSE) 
{
  ...
  p = .PermuteCovariateVectors2(observedTime, notCensored, count, 
                               I, covArray, betas)
  if (groupByD) {
...
.PermuteCovariateVectors2 <- function (t, d, count, I, covArray, betas) 
{
  n = sum(count[I])
  p <- integer(n)
  v <- seq(n)
  ...
  for (k in I) {
    if (d[k]) {
      ...
      J = sample(length(v), size = count[k], replace = FALSE, 
                 prob = .partialHazards2(t[k], v, covArray, betas))
    }
    else {
      # Censored
      J = sample(length(v), size = count[k], replace = FALSE, 
...
.partialHazards2 <- function (t, v, covArray, betas) 
{
  if (is.matrix(betas)) {
